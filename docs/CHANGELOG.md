# Changelog

Todos los cambios relevantes de este proyecto se documentan aqui.

El formato sigue **Keep a Changelog** (adaptado al espanol) y versionado semantico.

## [Unreleased]

### Added
- Documentacion tecnica estandarizada por modulos en `docs/modules/`.
- ADRs iniciales para flujo de ramas y manejo de assets publicos.
- Workflow CI basico en GitHub Actions (install + build).

### Changed
- README reemplazado por descripcion tecnica del backend y variables de entorno.
- Se formaliza trazabilidad de cambios con release notes y changelog en `docs/`.

## [1.1.0-baseline2] - 2026-02-24

### Added
- Baseline de documentacion y trazabilidad del backend (README, changelog, release notes, ADRs, docs de modulos).
- Estandar de ramas GitFlow-lite (`main`, `develop`, `release/*`, `feature/*`).
- CI basico para `main` y `develop`.

### Documentation
- Guia de resolucion de conflictos documentada en `docs/conflicts.md`.
- Documentacion por modulo/controlador generada automaticamente y preparada para enriquecimiento manual.

